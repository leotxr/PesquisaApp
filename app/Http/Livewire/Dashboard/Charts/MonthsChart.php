...
class MonthsChart extends Component
{
    ...
    public function getLastMonthsWithCount(int $start_sub_days, int $end_sub_days, int $max_check)
    {
        ...
        $count = Rating::whereMonth('created_at', $date->format('m'))->whereYear('created_at', $date->format('Y'))->count(); //Retorna a quantidade de pesquisas respondidas na data recuperada
